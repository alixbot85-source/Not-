...
پیاده کرده و یک HTTP API محلی در اختیار می‌گذارد.
...
cd ~
git clone https://github.com/ghaemifard/EitaaBun
cd EitaaBun && bun install
```

### اجرا (در یک session جدا)

```bash
cd ~/EitaaBun && bun run index.ts
```

روی `http://localhost:1234` بالا می‌آید. آن را باز بگذارید.

> در ترموکس برای داشتن دو پنجرهٔ همزمان، از منوی کناری `New session`
> استفاده کنید یا `tmux` نصب کنید.
...
سپس ربات را دوباره اجرا کنید. حالا در «مدیریت اکانت‌ها» گزینهٔ
**📱 ورود با شمارهٔ تلفن** فعال می‌شود:
